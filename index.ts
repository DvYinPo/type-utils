 ...
 */
export type TrimLeft<T extends string> = T extends `${infer space}${infer K}`
  ? TrimLeft<K>
  : T;
 ...
 */
export type MyCapitalize<S extends string> = S extends `${infer x}${infer tail}`
  ? `${Uppercase<x>}${tail}`
  : S;
...
export type StringToUnion<
  T extends string,
  K = never
> = T extends `${infer F}${infer R}` ? StringToUnion<R, F | K> : K;
...
export type Diff<T, R> = {
  [Key in keyof (T & R) as Key extends keyof (T | R)
    ? never
    : Key]: Key extends keyof T ? T[Key] : Key extends keyof R ? R[Key] : never;
};
 ...
 */
export type StartsWith<T, U extends string> = T extends `${U}${infer R}`
  ? true
  : false;
